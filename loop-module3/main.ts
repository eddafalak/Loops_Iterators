//define the type for a person in the data set 
type Person = {
    id: number;
    name: string;
    birthdate: string;
...
// used a forEach loop and ised a ternary operator to convert the boolean can_program value to Yes or no
people.forEach(person => {
    console.log(`name: ${person.name} Birth Date: ${person.birthdate} Children: ${person.birthdate} Country: ${person.country} Can program: ${person.can_program ? 'Yes' : 'No'}`);
});